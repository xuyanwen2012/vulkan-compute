...
namespace core
{
	...
	public:
		...
		explicit Buffer(const vk::DeviceSize size,
		...
		~Buffer()
		{
			...
			{
				vmaDestroyBuffer(g_allocator, get_handle(),
				                 allocation_);
			}
		}
		...
		void update(const std::vector<std::byte>& data, const size_t offset = 0) const
		{
	...
	public:
		vk::Buffer& get_handle() { return buffer_; }
		const vk::Buffer& get_handle() const { return buffer_; }
	};
} // namespace core

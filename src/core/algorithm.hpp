...
public:
  ...
  ~Algorithm() override {
    std::cout << "[DEBUG] Algorithm destructor" << std::endl;
    destroy();
  }
  ...
  void rebuild(
    ...
    if (!specialization_constants.empty()) {
      ...
      constexpr uint32_t memory_size =
          sizeof(decltype(specialization_constants.back()));
      ...
      std::memcpy(this->m_specialization_constants_data_,
                  specialization_constants.data(), total_size);
    ...
    if (!push_consts.empty()) {
      if (m_push_constants_data_) {
        free(m_push_constants_data_);
      }
      uint32_t memorySize = sizeof(decltype(push_consts.back()));
      uint32_t size = push_consts.size();
      uint32_t totalSize = size * memorySize;
      this->m_push_constants_data_ = malloc(totalSize);
      std::memcpy(this->m_push_constants_data_, push_consts.data(), totalSize);
      this->m_push_constants_data_type_memory_size_ = memorySize;
  ...
  void set_push_constants(const void *data, const uint32_t size,
    ...
    if (const uint32_t previous_total_size =
            ...
            this->m_push_constants_data_type_memory_size_;
...
protected:
  ...
  void create_parameters() {
    ...
    // TODO: -------------------- tmp --------------
    std::array tmp_buffer{Buffer(device_ptr_, 1024), Buffer(device_ptr_, 1024)};
...
public:
  void destroy() override {
    std::cout << "[DEBUG] Algorithm destroy" << std::endl;
